...
description:    train and validation script for image classification (3D)
'''
...
def train(data_object, readbatchsize=1, readshufflesize=1, vbatchsize=1, \
    ...
    print "Declare model"
    # Output from model
    y_pseg = model.image_classification_model(tof_in, masksize, training, kprob)
    y_psfmax = tf.nn.softmax(y_pseg, dim=-1)
    sys.stdout.flush()

    # Training vars
    train_accu_seg = costfunction.pixe_wise_accuracy(y_aseg, y_pseg, axis=-1)
    train_cost = costfunction.weightedFocalCrossEntropyLoss(y_aseg, y_pseg, \
    ...
    train_accumulator = costAccumulator(["train_cost", "train_accu"])
    valid_accumulator = costAccumulator(["valid_cost", "valid_accu"])
    ...
    saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='ImageClassificationModel'))
    with tf.Session(config = tf.ConfigProto(gpu_options = gpu_options)) as sess:
        ...
        while (iepoch < max_epochs):
            ...
            tensorboard_feeder = {track_acc:train_accumulator.getValues()[1], \
                                track_loss:train_accumulator.getValues()[0]}
            log1, log2 = sess.run([tracking_acc, tracking_loss], feed_dict=tensorboard_feeder)
            ...
            if (validate_every_nepoch > 0 and iepoch % validate_every_nepoch == 0) or (iepoch == max_epochs-1):
                ...
                tensorboard_feeder = {track_acc:valid_accumulator.getValues()[1], \
...
def predict(data_object, vbatchsize=1, out_res_path=None, restore_path='tfgraph'):
    ...
    # Training vars
    train_accu_seg = costfunction.pixel_wise_accuracy(y_aseg, y_pseg, axis=-1)
    train_cost = costfunction.weightedFocalCrossEntropyLoss(y_aseg, y_pseg, \
        weighting=True, weights=sweights, focal=True, gamma=2)

    # Object to accumulate costs etc.
    valid_accumulator = costAccumulator(["test_cost", "test_accu"])
    ...
    with tf.Session(config = tf.ConfigProto(gpu_options = gpu_options)) as sess:
        ...
        while (True):
            ...
            for kk in range(y_aseg_sb.shape[0]):
                actualList.append(y_aseg_sb[kk])
                heatList.append(y_pred_sb[kk])

            istep += 1

        #finding tp, fp
        predictions = np.argmax(heatList, axis=1)
        truths = np.argmax(actualList, axis=1)
        accu_modified = np.sum(predictions==truths)/float(len(truths))
        TP = 0
        TN = 0
        FP = 0
        FN = 0
        for i in range(len(predictions)):
            if predictions[i]==1 and truths[i]==1:
                TP +=1
            if predictions[i]==0 and truths[i]==0:
                TN +=1
            if predictions[i]==0 and truths[i]==1:
                FN +=1
            if predictions[i]==1 and truths[i]==0:
                FP +=1
        recall_val = float(TP)/float(TP+FN)
        prec_val = float(TP)/float(TP+FP+0.00000001)
        f1 = (2.0 * prec_val * recall_val)/(prec_val + recall_val + 0.00000001)
